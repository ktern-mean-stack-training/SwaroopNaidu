...
app.post('/adduser', (req, res)=> {          //defining the route to handle the post request
  let user = req.body;                   // getting the student details from the request body
  let userId = userdetails.length + 1;   // assigning a unique id to the student
  user.id = userId;                      // adding the id property to the student object
...
//3.deleting a single user -- Done

app.delete("/userdelete/:userid",(req,res)=>{   // route to handle delete request 
  let userid = parseInt(req.params.userid);      //converting user id from url to integer using parseInt
  for (let i=0; i<userdetails.length;i++){      // looping through the userdetails array to find a match with the requested user ID
    console.log("loop is running")
    if (userdetails[i].id===userid){            // if the user ID in the request matches with a user ID in the userdetails array
      userdetails.splice(i,1);                  // remove the user object from the array using splice
      break                                     // break the loop since the user has been found and removed
    }
    console.log("id not found")
  }
  res.send(userdetails)                         // send the updated userdetails array as a response to the client
})
...
app.get("/userdata",(request,res)=>{                 //defining route to andle the get request
